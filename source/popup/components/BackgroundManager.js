...
export default {
  template: html`
    <div class="component-wrapper">
      ...
      <option-toggle storageKey="transparentBgEnabled" mode="show">
        <hr />
        <label for="blurOpt">Blur option:</label>
        <input type="text" id="blurOpt" v-model="blurOpt">
      </option-toggle>
  ...
  data: () => ({
    ...
    cache: "--color=#00FFFFFF --enable=true --win-blur=blurbehind --mac-vibrancy=fullscreen-ui --blur-corner-radius=0 --gnome-sigma=100"
  }),
  components: {
    BackgroundEmbed,
    OptionToggle
  ...
  computed: {
    ...
    blurOpt: {
      get() {
        browser.storage.local.get("blurArgs").then(result => {
          let val = result["blurArgs"];
          if (this.cache !== val)
            this.cache = val;
        });
        return this.cache;
      ...
      set(val) {
        browser.storage.local.set({ ["blurArgs"]: val }).then(() => {
          this.cache = val;
        });
      }
    }
